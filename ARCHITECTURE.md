...
- `buildStatsMarkdown()` — markdown для `STATS.md`: Mermaid-график `xychart-beta` за последние 30 календарных дней (X — дни, Y — количество; GitHub рендерит её нативно), при данных более чем за один месяц — секция «По месяцам» (график + таблица сумм), затем таблица «дата → количество» по всем дням + строка «Итого».
